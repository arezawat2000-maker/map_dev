...
class UserProfileRepository {
  ...
  /// Ensures `users/{email}` exists so admins can list this account.
  ...
  Future<void> saveProfile(UserProfile profile) async {
    final emailId = normalizeEmailId(profile.email);
    final ref = _profileRef(emailId);
    final existing = await ref.get();
    final data = profile.copyWith(email: emailId).toMap(
          includeCreatedAt: !existing.exists,
        );
    // Never clear an existing ban via normal profile save.
    if (existing.exists) {
      data.remove('banned');
      data.remove('bannedAt');
      data.remove('bannedBy');
    }
    await ref.set(data, SetOptions(merge: true));
    // Parent `users/{email}` is best-effort (collectionGroup listing does not need it).
    try {
      await ensureUserListed(emailId, profile: profile.copyWith(email: emailId));
    } catch (_) {}
  }

  /// True when profile exists and username is non-empty.
  Future<bool> hasCompletedOnboarding(String emailId) async {
    final profile = await getProfile(emailId);
